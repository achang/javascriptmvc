 ...
 * @class
 * <pre>Example:
 *      Examples = Class.create(Junction.ActiveRecordGenerator('Examples'), {.....} )
 * </pre>
 */
Junction.ActiveRecordGenerator = function(model_name) {
	window[model_name] = Class.create(Junction.ActiveRecord, {klass_name: model_name});
    ...
    var find_table_in_schema = function(table_name) {
        if(Junction.app_schema) {
            for (var i=0; i<Junction.app_schema.tables.length; i++) {
                var table = Junction.app_schema.tables[i];
	...
	var lookup_table = function(model_name) {
		...
		for(var table_name in Junction.ActiveRecord.table_names_hash) {
			if(Junction.ActiveRecord.table_names_hash[table_name] == model_name) {
		...
		var table_object = find_table_in_schema(table_name);
		if(table_object) {
			Junction.ActiveRecord.table_names_hash[table_name] = model_name;
			return table_object;
		}
		// Thing --> Things
		...
		throw new Junction.Error(new Error(), 'Model not found in the application schema: '+model_name);
	}
	...
	var generate_columns_hash = function() {
	    ...
	    for (var j=0; j<table_object.columns.length; j++) {
	        ...
	        columns_hash[column.name] = new Junction.ColumnController(column.name, column.type, column['null'])
	    }
	...
	Junction.junction_startup_tasks.push(generate_columns_hash);
 ...
 */
Junction.ColumnController = function(name, sql_type, is_null, default_value) {
	return new Junction[sql_type.capitalize()+'Column'](name, sql_type, is_null, default_value)
}


Junction.Column = function() {}
Junction.Column = Class.create( 
Junction.Column.prototype = {
	...
	toString : function() {
		return '<Junction.Column #'+this.name+'>'	
	},
	inspect : function() {
		return 'Junction.Column: name='+this.name+', sql_type='+this.sql_type+', is_null='+this.is_null
	}
} 
)
 ...
 * @class
 */
Junction.StringTextColumn = function(){}
Junction.StringTextColumn = Class.create(Junction.Column, {
	to_sql: function(value) {
		if(Junction.database_adapter == 'msaccess') {
 ...
 * @class
 */
Junction.StringColumn = function(){}
Junction.StringColumn = Class.create(Junction.StringTextColumn)

/**
 * @class
 */
Junction.TextColumn = function(){}
Junction.TextColumn = Class.create(Junction.StringTextColumn)

/**
 * @class
 */
Junction.IntegerColumn = function(){}
Junction.IntegerColumn = Class.create(Junction.Column, {
	typecast_attribute: function(value) {
 ...
 * @class
 */
Junction.DecimalColumn = function(){}
Junction.DecimalColumn = Class.create(Junction.IntegerColumn, {
	typecast_attribute: function(value) {
 ...
 * @class
 */
Junction.DatetimeColumn = function(){}
Junction.DatetimeColumn = Class.create(Junction.Column, {
	typecast_attribute: function(value) {
 ...
 * @class
 */
Junction.DateColumn = function(){}
Junction.DateColumn = Class.create(Junction.DatetimeColumn)
 ...
 * @constructor
 * @class
 * <p>Junction.ActiveRecord is based on the active record design pattern where a database table is wrapped into a
 * class.  An object instance is tied to a single row in the table.
 * Junction.ActiveRecord objects don‘t specify their attributes directly, 
 ...
 * Any change is instantly reflected in the Junction.ActiveRecord objects. </p>
 * 
 * <h4>Definition</h4>
 * <p>Example class that inherits functionality from Junction.ActiveRecord:</p>
 * <pre class='example'>
 *  Task = Class.inherit(Junction.ActiveRecordGenerator('Task'), {  
 *      //add class methods here
 *  });</pre>
 * 
 * <h4>Creation</h4>
 * <p>Junction.ActiveRecord accepts constructor parameters as a hash:</p>
 * <pre class='example'>
 * task = Task.new_instance( {title: <span>"Mow Grass"</span>, description: <span>"1. Fill with gas.\n2. Start.\n3. Mow"</span> } )</pre>
 *
 ...
 */
Junction.ActiveRecord = function(){}
Junction.ActiveRecord.prototype = {
    /**
    ...
    destroy : function(options) {
        ...
        return Junction.QueryController.send(destroy_template.evaluate(template_data));
    },
		...
		if(attributes && typeof(attributes)=='object')
			for(var attribute in attributes) {
				if(typeof this[attribute] != 'undefined' || attribute == 'id')
					...
					throw(new Junction.Error(new Error(), 'unrecognized attribute '+attribute+' for model '+this.klass_name))
			}
    ...
    save : function(request_params) {
		...
		var updated_at_column = this.klass().columns_hash()['updated_at'] instanceof Junction.DatetimeColumn;
		var created_at_column = this.klass().columns_hash()['created_at'] instanceof Junction.DatetimeColumn;
		...
		if(this.is_new_record() && this.id) {  // do a literal insertion
            ...
            return Junction.QueryController.send(insert_template.evaluate(template_data), request_params || {});
		}
            ...
            return Junction.QueryController.send(update_template.evaluate(template_data), request_params || {});
		}
    },
...
Junction.ActiveRecord = Class.create(Junction.ActiveRecord.prototype);
 ...
 * <p>Example:</p>
 * <pre>
 * Junction.ActiveRecord._sql_column_name('name') => 'employees.name'
 * </pre>
 * @private
 * @param {Object} name
 */
Junction.ActiveRecord._sql_column_name = function(name) {
 ...
 */
Junction.ActiveRecord.associations = function() {
	if(Junction.ActiveRecord._associations_are_setup == false) {
		Junction.ActiveRecord.setup_associations();
	}
 ...
 *      </ul>
 */
Junction.ActiveRecord.belongs_to = function(association_name, info) {
 ...
 */
Junction.ActiveRecord.column_names = function() {
 ...
 * <p>Example:
 * <pre class='example'>
 * Computer.columns() => [Junction.Column #id, Junction.Column #ip_address, Junction.Column #user]</pre>
 * @return {Array} Array of column names.
 */
Junction.ActiveRecord.columns = function() {
 ...
 * <p>Example:
 * <pre class='example'>
 * Computer.content_columns() => [Junction.Column #ip_address, Junction.Column #user]</pre>
 * @return {Array} Array of column names.
 */
Junction.ActiveRecord.content_columns = function() {
...
*/
Junction.ActiveRecord.count = function(params) {
	params = params || {};
    ...
    return Junction.QueryController.send(sql, params);
}
 ...
 */
Junction.ActiveRecord.create = function(object, params) {
 ...
 */
Junction.ActiveRecord.create_alias = function(from_models) {
	var sql_arr = [];
 ...
 * @param {Object} info
 */
Junction.ActiveRecord.create_belongs_to = function(association_name, info) { 
	if(!info) {
		info = {};
	...
	if(!(this.columns_hash()[info.foreign_key]
		&& this.columns_hash()[info.foreign_key] instanceof Junction.IntegerColumn)) {
			throw(new Junction.Error(new Error(), 'Foreign Key must be an integer type'));
	}
 ...
 * @param {Object} info
 */
Junction.ActiveRecord.create_has_many = function(plural_association_name, info) {
	if(!info) {
		info = {};
	...
	if(!(window[info.model_name].columns_hash()[info.foreign_key] 
		&& window[info.model_name].columns_hash()[info.foreign_key] instanceof Junction.IntegerColumn)) {
			throw(new Junction.Error(new Error(), 'Foreign Key must be an integer type'));	
	}
 ...
 */
Junction.ActiveRecord.destroy = function(id) {
	if(typeof(id) == 'object') {
		id.each(function(id) {
			if(! this.destroy(id))
 ...
 */
Junction.ActiveRecord.destroy_all = function(conditions) {
 ...
 */
Junction.ActiveRecord.exists = function(id_or_conditions) {
 ...
 *  </table>
 * @return {Junction.Request} an array of objects of the requesting class.
 */
Junction.ActiveRecord.find = function(type, params) {
 ...
 */
Junction.ActiveRecord.find_by_sql = function(sql, params, find_params) {
...
Junction.ActiveRecord.packaged_find = function(type, params) {
	params = params || {};
	params.no_send = true;
 ...
 *  </table>
 */
Junction.ActiveRecord.has_many = function(association_name, info) {
 ...
 * @return {Class} the class for the given instance
 */
Junction.ActiveRecord.klass = function(){
    if(this.klass_name)
        return window[this.klass_name];
}
/**
 ...
 */
Junction.ActiveRecord.load_from_schema = function(schema_source) {
    if(!schema_source || !APPLICATION_ROOT) 
		throw new Junction.Error(new Error(), 'schema_source or APPLICATION_ROOT not defined');
    ...
    Junction.app_schema = applicationinfo;
    for (var i=0; i<Junction.app_schema.tables.length; i++) {
		var file_name = Junction.ActiveRecord.table_name_to_model_name(Junction.app_schema.tables[i].name).uncapitalize();
		try {
	        include(JITS.File.join(APPLICATION_ROOT, "app", "models", file_name+".js"), {cache: (Junction.ENV.ENVIRONMENT == 'development' ? false : true)  });
 ...
 */
// 
Junction.ActiveRecord.process_conditions = function(conditions) {
    if(conditions) {
 ...
 */
Junction.ActiveRecord.process_find_results = function(find_params) {
	...
	return function(object_hash) {
			...
			if(window[base_model_name].associations().belongs_to[association_name] || 
			  window[base_model_name].associations().has_many[association_name]) {
			  	for(var i=0; i<objects_hash[base_model_name].length; i++) {
					...
					if(association_data)
						Junction.QueryController.add_to_cache(association_data.sql);
				}
			}
		...
		var cache_object = function(object, include_arr) {
			...
			Junction.QueryController.add_to_cache(finder_sql);
		}.bind(this)
	    ...
	    for(var table in object_hash) {
	        var model = Junction.ActiveRecord.table_name_to_model_name(table);
 ...
 * @param {Object} find_params
 */
Junction.ActiveRecord.process_includes = function(includes, find_params) {
 ...
 * @param {Object} find_params
 */
Junction.ActiveRecord.process_includes_conditions = function(includes, conditions, find_params) {
    includes = includes || [];
 ...
 * @param {Object} offset
 */
Junction.ActiveRecord.process_limit_offset = function(limit, offset) {    
	if(offset!=null && Junction.database_adapter == 'msaccess')
		throw new Junction.Error(new Error(), 'offset is not a defined operation in MS Access');
	if(limit != null && Junction.database_adapter == 'msaccess') {
 ...
 * @param {Object} order
 */
Junction.ActiveRecord.process_order = function(order) {
    if(!order) return ''
 ...
 */
Junction.ActiveRecord.set_table_name = function(table_name) {
	Junction.ActiveRecord.table_names_hash[table_name] = this.klass_name;
}
/**
 * Sets up associations for all the models.  This is called when the first association is tried.
 * @private
 */
Junction.ActiveRecord.setup_associations = function(){
    // set up associations
    for (var i=0; i<Junction.app_schema.tables.length; i++) {
        var model_name = Junction.ActiveRecord.table_name_to_model_name(Junction.app_schema.tables[i].name);
		...
		Junction.ActiveRecord._associations_are_setup = true;
    }
}
 ...
 */
Junction.ActiveRecord.sum= function(column_name, params) {
    ...
    if(params.no_send && params.no_send == true)
        return {sql: sql, params: params};
    return Junction.QueryController.send(sql_query_array.join(' '), params);
}

// this function returns the model name for the given table_name
// after all of Junction's models have been inititialized, this function is guaranteed to return the correct model
 ...
 */
Junction.ActiveRecord.table_name_to_model_name = function(table_name) {
	...
	if(Junction.ActiveRecord.table_names_hash[table_name])
		return Junction.ActiveRecord.table_names_hash[table_name];
...
*
*/
Junction.ActiveRecord.update = function(id, attributes, request_params) {
	if(typeof(id) == 'object') {
		var idx = -1;
 ...
 * @param {Object} params
 */
Junction.ActiveRecord.update_all = function(updates, conditions, params) {
	var UpdateGrammar = {}
 ...
 * @param {Object} column
 */
Junction.ActiveRecord.typecast_attribute = function(value, column) {
