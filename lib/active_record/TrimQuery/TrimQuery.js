...
(function() { // Using a closure to keep global namespace clean.
        ...
        var data_insertion = function(table_info, field_name, data, column_ref) {
            if(table_info[field_name]) {
				if(/null/i.exec(data))
	                ...
	                data = eval(data);
                if(data && table_info[field_name].type && table_info[field_name].type == 'Number')
                    data = Number(data, 10);
        ...
        var NodeType = { // Constructor functions for SELECT statement tree nodes.
            select : function(args) {
                ...
                this.filter = function(dataTables, bindings, params) {
                    ...
                    if (groupByCalcValues != null) {
                        ...
                        resultOfFromWhere.sort(group_by_comparator);
                    }
                    ...
                    for (var i = 0; i < resultOfFromWhere.length; i++) {
                        ...
                        if (prevItem != null &&
                            group_by_comparator(prevItem, currItem) != 0) {
            ...
            destroy  : function(args) {
                ...
                this.filter  = function(dataTables, bindings) {
                    ...
                    for(var table_name in dataTables) {
                        dataTables[table_name] = dataTables[table_name].compact();
                    }
    ...
    var compileColumnConvertor = function(tables, columnExpressions) {
        ...
        funcText.push(table_aliases.uniq().join(', '));
    ...
    QueryLang.prototype.parseSQL = function(sqlQueryIn, paramsArr) { // From sql to tql.
		...
		/*if(Junction.database_adapter == 'msaccess') {
			// weird access method for quoting within quotes
			// ... but have to avoid the empty string case
        ...
        if (!['SELECT', 'DESTROY', 'UPDATE', 'INSERT'].include(query_type))
        ...
        if (query_type == 'SELECT' || query_type == 'DESTROY') {
            ...
            for(var i=0; i<fields.length; i++) {
                object_hash[fields[i]] = values[i];
            }
            tql = ['INSERT(', table_name, ',', $H(object_hash).toJSON(), ')'];
            ...
            var values = QueryLang.Grammar.update_list(assignmentClause)[0];
