Junction.Query = function(sql) {
	this.sql = sql;
}

Junction.Query.prototype = {
    ...
    add_pair_into_insert : function(column, value) {
		if(this.query_type() != 'INSERT')
			throw new Junction.Error(new Error(), 'add_pair_into_insert called on a non-insert query');
...
Junction.Query.generate_insert = function(object, table) {	        
    var fields_array = [];
    var values_array = [];
	...
	return new Junction.Query('INSERT INTO '+table+' ('+fields_string+') VALUES ('+values_string+')');
}