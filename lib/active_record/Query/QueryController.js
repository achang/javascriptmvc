    ...
    send : function(sql, params) {
        ...
        if(query.query_type() == 'INSERT') {
            var querycontroller_callback = function(last_id) {
                ...
                Junction.ClientInterface.send_query(query.sql);
                return last_id;
            }.bind(this)
			if( params.force_local )
            	return Junction.ClientInterface.send_query(sql);
            return Junction.ServerInterface.send_query(sql, params, querycontroller_callback);
        } else if (query.query_type() == 'SELECT') {
            if( params.force_local || (this._query_cache_hit(query) && !params.force_remote) ) {
                return Junction.ClientInterface.send_query(sql, params.callback);
            } else {
				var callback = this._copy_objects_to_client_db;
				if(sql.match(/COUNT\(\*\)|SUM\(.*\)/)) // no need to copy objects for these queries
					callback = null;
                return Junction.ServerInterface.send_query(sql, params, callback);
            }
        } else {
			if( params.force_local ) {
            	Junction.ClientInterface.send_query(sql);
				return;
			} else {
	            Junction.ClientInterface.send_query(sql);
	            Junction.ServerInterface.send_query(sql, params);
	            return;
			}
        }
    },
    ...
    _copy_objects_to_client_db : function(object_hash) {
		for(var table in object_hash) {
			for(var primary_value in object_hash[table]) {
	            ...
	            var insert = Junction.Query.generate_insert(object, table);
	            Junction.ClientInterface.send_query(insert.sql);
			}
		}
    }
}