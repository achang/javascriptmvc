...
Junction.View.sorter_link = function(sorter, name, title){
	title = title || name;
	if( title.include('.') ) {
	...
	if(! direction )
		return Junction.View.link_to(title, sorter.params_with(name, "ASC")  )
	else if( direction ==  'DESC'  )
		return (sorter.is_first(name)? '&#x25B3;' : '&#x25B4;')+Junction.View.link_to(title, sorter.params_with(name, "ASC")  )
	else
		return (sorter.is_first(name)? '&#x25BC;' : '&#x25BF;')+Junction.View.link_to(title, sorter.params_with(name, "DESC")  )
}
