...
JMVC.View.DEFAULT_PAGINATION_OPTIONS = {
	link_to_current_page : false,
	always_show_anchors : true,
	window_size : 3,
	name : 'page'
}
JMVC.View.pagination_links = function(paginator, options, html_options) {
	options = $(JMVC.View.DEFAULT_PAGINATION_OPTIONS).merge(options)
	var params = Object.extend({},paginator.controller.params) // shallow copy
	
	
	return JMVC.View.pagination_links_each(paginator, options, function(n){
		params[options.name] = n;
		JMVC.View.link_to(n, params, html_options)
	})
}


JMVC.View.pagination_links_each = function(paginator, options, block) {
	options = $(JMVC.View.DEFAULT_PAGINATION_OPTIONS).merge(options)
