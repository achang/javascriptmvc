...
FilterParam.prototype = {
	get_class : function() {
		var class_name = Junction.ActiveRecord.table_name_to_model_name(this.table_name);
...
Junction.View.filter_dropdown = function(filter, name, options){
...
Filter.change_url = function(options) {
	...
	Junction.Routes.change_url(options);
}

Junction.Routes.change_url = function(options) {
	window.location = window.location.pathname+"#"+Junction.Routes.url_for(options);
}
