JMVC.ActionViewGenerator = function(model, name, proto) {
	...
	var new_class_name = name.capitalize() //AR_Todo.View.Show

	model.View[new_class_name] = Class.create(JMVC.ActiveView, proto); 
	var new_class = model.View[new_class_name]
...
JMVC.ActiveView = Class.create({
	initialize : function(object) {
		...
		this.klass.get_columns().each(function(column_name){
			...
			if(column){
				var view_klass = object.klass().View[this.klass.view_name.capitalize()]
				var column_class
				if(view_klass)
					column_class = object.klass().View[this.klass.view_name.capitalize()][column_name.capitalize()] //AR_Todo.View.Show.Complete
				
				var sql_type_column_class = JMVC.ColumnView[column.sql_type.capitalize()]
				var created_column;
...
JMVC.ColumnView.Boolean.Edit = Class.create(JMVC.ColumnView.Boolean, {
...
JMVC.ColumnView.String = Class.create(JMVC.ColumnView, {
})
JMVC.ColumnView.String.Edit = Class.create(JMVC.ColumnView.String, {
	make_content : function(){
		var content=document.createElement('input')
		content.type='text'
