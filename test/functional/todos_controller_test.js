new $MVC.Test.Controller('todos',{
	   ...
	   click_todo_type_and_blur_part2 : function() {
		   ...
		   this.assert_equal($MVC.CSSQuery('.todo label')[0].innerHTML, "Learn JavaScriptMVChello world");
		   this.next(null, null, 'blank');
       },
       test_todo_edit_and_hit_enter : function(){
           var params = this.TodoLabelClick();
           var params = this.TodoLabelInputKeypress({character: 'D'});
		   this.assert_equal($MVC.CSSQuery('.todo label input')[0].value, "Learn JavaScriptMVChello worldD");
           var params = this.TodoLabelInputKeypress({keyCode: 13});
		   this.assert_equal($MVC.CSSQuery('.todo label')[0].innerHTML, "Learn JavaScriptMVChello worldD");
		   this.next(null, null, 'blank');
       },
		...
		test_focus_type_blur_input_box : function(){
		    ...
		    this.assert_equal($MVC.CSSQuery('.todo label')[0].innerHTML, "Walk the dog!!");
		   this.next(null, null, 'blank');
		},
		...
		submit_new_todo_part2 : function() {
		    ...
		    this.assert_equal($MVC.CSSQuery('.todo label')[0].innerHTML, "Brian");
		...
		test_delete_todo : function() {
			...
			this.assert_equal($MVC.CSSQuery('.todo').length, 2);
		},
		blank : function() {
			// do nothing
		}
