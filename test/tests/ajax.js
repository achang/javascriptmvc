	...
	perform_test : function() {
	  new Test.Unit.Runner({
	    ...
	    test_get_synch_no_params : function() { with(this) {
			var request = new Ajax.Request('files/ajax.html', {asynchronous: false, method: 'get'});
			assert("<p>Content</p>",request.transport.responseText)
	    }},
		test_get_synch_params : function() { with(this) {
			var request = new Ajax.Request('files/ajax.html', {asynchronous: false, method: 'get', parameters: {hello: 'world'}} );
			assert("<p>Content</p>",request.transport.responseText)
